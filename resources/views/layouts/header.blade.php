...
<html lang="en">
  ...
  <path d="M..." fill="none" stroke="currentColor" stroke-width="1.5"/>
</symbol>

<symbol id="heart-filled" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  <path fill="currentColor" d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 
    2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41 0.81 4.5 
...
if (Auth::check()) {
    // Logged-in: fetch from database with product info
    $cartItems = App\Models\CartItem::with('product')
        ...
        ->map(function ($item) {
            return [
                ...
                'volume_point' => $item->product->volume_point ?? '',
...
$volume_pointTotal = array_sum(array_map(function ($item) {
    return ($item['volume_point'] ?? 0) * ($item['quantity'] ?? 1);
}, $cartItems));
@endphp
    ...
    <div class="order-md-last">
      ...
      <ul class="list-group mb-3">
        ...
        <li class="list-group-item d-flex justify-content-between lh-sm">
          <div class="d-flex">
    @if (!empty($item['image']))
    <img src="{{ asset('storage/' . $item['image']) }}" alt="Product" width="50" class="me-2">
    @endif
          <div>
            <h6 class="my-0">{{ $item['name'] ?? 'na'}}</h6>
            <small class="text-muted">VP: {{ $item['volume_point'] ?? 'bb' }}</small><br>
            <small class="text-muted">Qty: {{ $item['quantity'] }}</small>
          </div>
          </div>
          <span class="text-muted">₹{{ number_format(($item['price'] ?? 0) * $item['quantity'], 2) }}</span>
        ...
        <li class="list-group-item d-flex justify-content-between">
          <span>Total VP</span>
          <strong>{{ number_format($volume_pointTotal, 2) }}</strong>
        </li>
      </ul>

      @if ($cartCount > 0)
      <a href="{{ route('checkout') }}" class="btn btn-primary w-100 btn-lg">Continue to Checkout</a>
      @else
      <button class="btn btn-secondary w-100 btn-lg" disabled>Cart is empty</button>
      @endif
    </div>
  </div>
</div>
...
$wishlistItems = App\Models\Wishlist::with('product')->where('user_id', Auth::id())->get();
        ...
        <li class="list-group-item d-flex justify-content-between lh-sm">
          <div class="d-flex">
            @if (!empty($product->image))
              <img src="{{ asset('storage/' . $product->image) }}" alt="Product" width="50" class="me-2">
            @endif
            <div>
              <h6 class="my-0">{{ $product->name }}</h6>
              <small class="text-muted">VP: {{ $product->volume_point }}</small>
              <br>
        ...
        <div class="row py-3 border-bottom">
            ...
            <ul class="navbar-nav list-unstyled d-flex flex-row gap-3 gap-lg-5 justify-content-center flex-wrap align-items-center mb-0 fw-bold text-uppercase text-dark">
              ...
              <li class="nav-item dropdown">
                ...
                <ul class="dropdown-menu border-0 p-3 rounded-0 shadow" aria-labelledby="pages">
                  ...
                  {{-- <li><a href="{{ route('cart.show1') }}" class="dropdown-item">Checkout </a></li> --}}
                  
                  <li><a href="index.html" class="dropdown-item">Contact </a></li>
                  
                  {{-- @if(Auth::User() && Auth::User()->role == 'admin')
                  <li><a href="{{ route('admin_dashboard') }}" class="dropdown-item">My Account </a></li>
                  @elseif(Auth::User() && Auth::User()->role == 'user')
                  <li><a href="{{ route('user_dashboard') }}" class="dropdown-item">My Account </a></li>
                  @else
                  <li><a href="{{ route('login') }}" class="dropdown-item">My Account </a></li>
                  @endif --}}
          ...
          <div class="col-sm-8 col-lg-2 d-flex gap-5 align-items-center justify-content-center justify-content-sm-end">
            <ul class="d-flex justify-content-end list-unstyled m-0">
              <li>
              @if(Auth::User() && Auth::User()->role == 'admin')
              <a href="{{ route('admin_dashboard') }}" class="p-2 mx-1">
              @elseif(Auth::User() && Auth::User()->role == 'user')
              <a href="{{ route('user_dashboard') }}" class="p-2 mx-1">
              @else
             ...
             <li>
            <a href="#" class="p-2 mx-1" data-bs-toggle="offcanvas" data-bs-target="#offcanvasWishlist">
